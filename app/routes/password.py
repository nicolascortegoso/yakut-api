...
from app.schemas import PasswordReset, PasswordResetRequest, db
from app.send_email import password_reset
...
async def reset_request(user_email: PasswordResetRequest, request: Request):
    user = await db["users"].find_one({"email": user_email.email})
    if user is not None:
        token = create_access_token({"id": user["_id"]})

        reset_link = f"{request.url.scheme}://{request.client.host}:{request.url.port}/reset?token={token}"
